...
echo "=== generating Kubernetes manifests with resource limits, secrets and ConfigMap... ==="
...
echo "=== creating ConfigMap for MongoDB host... ==="
cat > k8s-manifests/1-mongo-configmap.yaml <<EOF
apiVersion: v1
kind: ConfigMap
metadata:
  name: mongo-config
data:
  mongo_host: "microservice-db"
EOF
